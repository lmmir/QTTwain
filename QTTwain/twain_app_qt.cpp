      ...
      case TWFF_BMP:
        ...
        if((TWCP_NONE==twData.Compression)&&twData.BytesWritten)
        {
          ...
          else
          {
            ...
            int nDifference = 0 < nBytesPerRow-twData.BytesPerRow? 0: nBytesPerRow-twData.BytesPerRow;
            char *pExtra = NULL;
            if(nDifference)
            {
            ...
            for(int nRow = 0; nRow < twData.Rows; nRow++)
            {
              ...
              m_pMemoryDataStream->writeRawData(static_cast<char *>(twData.Memory.TheMem), (twData.BytesPerRow < nBytesPerRow?twData.BytesPerRow:nBytesPerRow));
              if(nDifference)
              {
...
void TWAIN_App_QT::TraceMessage(const char *pszFormatString, ...)
{
  ...
  {  
    ...
    //Output the result
    emit customAppendStatusText(QString::fromLocal8Bit(pszFormatted));
...
void TWAIN_App_QT::on_btnOpen_clicked(bool bChecked)
{
  ...
  OpenDS( ui.cbxSources->currentText().toLocal8Bit() );
  //Restore the cursor
...
void TWAIN_App_QT::on_btnEnable_clicked(bool bChecked)
{
  ...
  if(ui.radUIOnly->isChecked())
  {
    ...
    EnableDSUIOnly((TW_HANDLE)winId());
  }
  else
  {
    //Enable with intent to transfer UI or UIless
    EnableDS((TW_HANDLE)winId(), ui.radNoUI->isChecked()?false:true);
  }
  //Update the state of the GUI
  EnableGUIByCurrentState();
  return;
...
void TWAIN_App_QT::on_btnSetDefault_clicked(bool bChecked)
{
  ...
  if(TWRC_SUCCESS==GetDataSourceID(ui.cbxSources->currentText().toLocal8Bit(), twIdentity))
  {
...
void TWAIN_App_QT::PopulateTripletList()
{
  ...
  pItem->setText(0, QString::fromLocal8Bit(convertDataGroup_toString(DG_CONTROL)));
  ...
  pItem->setText(0, QString::fromLocal8Bit(convertDataGroup_toString(DG_IMAGE)));
...
{
  TraceMessage(strText.toLocal8Bit());
  return;
}
