  ...
  if(NULL==pfnCvrt)
  {
    switch(twItemType)
    {
      ...
      case TWTY_STR32:
        ...
        SSTRCPY(static_cast<char *>(pValue), 32, pItem->text().toLocal8Bit());
        pItem->setText(static_cast<char *>(pValue));
        break;
      case TWTY_STR64:
        memset(static_cast<char *>(pValue), 0, sizeof(TW_STR64));
        SSTRCPY(static_cast<char *>(pValue), 64, pItem->text().toLocal8Bit());
        pItem->setText(static_cast<char *>(pValue));
        break;
      case TWTY_STR128:
        memset(static_cast<char *>(pValue), 0, sizeof(TW_STR128));
        SSTRCPY(static_cast<char *>(pValue), 128, pItem->text().toLocal8Bit());
        pItem->setText(static_cast<char *>(pValue));
        break;
      case TWTY_STR255:
        memset(static_cast<char *>(pValue), 0, sizeof(TW_STR255));
        SSTRCPY(static_cast<char *>(pValue), 255, pItem->text().toLocal8Bit());
        pItem->setText(static_cast<char *>(pValue));
        break;
      case TWTY_HANDLE:
        break;
